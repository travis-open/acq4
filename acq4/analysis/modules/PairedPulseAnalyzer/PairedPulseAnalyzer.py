...
import PPControlTemplate, PPPlotsTemplate
...
class PairedPulseAnalyzer(AnalysisModule):

    dbIdentity = "PairedPulseAnalyzer"

    def __init__(self, host):
        AnalysisModule.__init__(self, host)

        self.ctrlWidget = QtGui.QWidget()
        self.ctrl = PPControlTemplate.Ui_Form()
        self.ctrl.setupUi(self.ctrlWidget)

        tables = OrderedDict([
            (self.dbIdentity+'.trials', 'PairedPulse_Trials'),
            (self.dbIdentity+'.cell', 'PairedPulse_Cell')
        ])
        self.dbGui = DatabaseGui(dm=self.dataManager(), tables=tables)

       
        self.plotsWidget = QtGui.QWidget()
        self.plots = PPPlotsTemplate.Ui_Form()
        self.plots.setupUi(self.plotsWidget)

        #self.pairingPlot = pg.PlotWidget()

        #self.fileLoader = FileLoader.FileLoader(self.dataManager(), host=self, showFileTree=True)
        ...
        self._elements_ = OrderedDict([
            ('File Loader', {'type':'fileInput', 'size': (100, 100),'host': self, 'showFileTree': False}),
        ...
        self.pspRgn2 = pg.LinearRegionItem(brush=(150,150,0,50))
        self.plots.tracesPlot.addItem(self.pspRgn2)
        ...
        self.ctrl.pspCheck_2.toggled.connect(self.regionDisplayToggled)
        ...
        self.ctrl.pspStartSpin_2.setOpts(suffix='s', siPrefix=True, dec=True, step=1, minStep=0.001)
        self.ctrl.pspEndSpin_2.setOpts(suffix='s', siPrefix=True, dec=True, step=1, minStep=0.001)
        self.ctrl.pspStartSpin_2.valueChanged.connect(self.pspSpinChanged2)
        self.ctrl.pspEndSpin_2.valueChanged.connect(self.pspSpinChanged2)
        self.pspRgn2.sigRegionChangeFinished.connect(self.pspRgnChanged2)
        ...
        self.pspRgn2.setRegion((0.062,0.07))
        self.healthRgn.setRegion((0.2,0.3))
    ...
    def clearTracesPlot(self):
        ...
        for item in [self.baselineRgn, self.pspRgn, self.pspRgn2, self.healthRgn]:
    ...
    def updateTracesPlot(self):
        ...
        if self.analysisResults is not None and len(data[dataKey] > 0):
            ...
            for i, time in enumerate(data[timeKey]):
                slopeTime = self.analysisResults[self.analysisResults['time'] == time]['highSlopeLocation1']
                ...
                slopeTime = self.analysisResults[self.analysisResults['time'] == time]['highSlopeLocation2']
                slopeInd = int(slopeTime/timestep)
                self.plots.tracesPlot.plot([slopeTime], [data[i][dataKey]['primary'][slopeInd]], pen=None, symbol='o', symbolPen=None, symbolBrush=pg.intColor(i, len(data)))
    ...
    def regionDisplayToggled(self):
        ...
        if self.ctrl.pspCheck_2.isChecked():
            self.pspRgn2.show()
        else:
            self.pspRgn2.hide()
    ...
    def pspRgnChanged2(self):
        try:
            self.ctrl.pspStartSpin_2.blockSignals(True)
            self.ctrl.pspEndSpin_2.blockSignals(True)
            rgn = self.pspRgn2.getRegion()
            self.ctrl.pspStartSpin_2.setValue(rgn[0])
            self.ctrl.pspEndSpin_2.setValue(rgn[1])
        except:
            raise
        finally:
            self.ctrl.pspStartSpin_2.blockSignals(False)
            self.ctrl.pspEndSpin_2.blockSignals(False)

    def pspSpinChanged2(self):
        try:
            self.pspRgn2.blockSignals(True)
            start = self.ctrl.pspStartSpin_2.value()
            end = self.ctrl.pspEndSpin_2.value()
            self.pspRgn2.setRegion((start, end))
        except:
            raise
        finally:
            self.pspRgn2.blockSignals(False)
    ...
    def analyze(self):
        ...
        self.analysisResults = np.zeros(len(traces), dtype=[('time', float), 
                                                            ('RMP', float), 
                                                            ('pspSlope1', float),
                                                            ('slopeFitOffset1', float),
                                                            ('highSlopeLocation1', float),
                                                            ('pspAmplitude1', float),
                                                            ('pspSlope2', float),
                                                            ('slopeFitOffset2', float),
                                                            ('highSlopeLocation2',float),
                                                            ('pspAmplitude2', float),
        ...
        if self.ctrl.pspCheck.isChecked():
            ...
            self.plots.plasticityPlot.plot(x=times-self.expStart, y=self.analysisResults['pspSlope2']/self.analysisResults['pspSlope1'],
                pen=None, symbol='o', symbolSize=symsize, symbolPen=None)
            self.plots.plasticityPlot.setLabel('left', "PairedPulseRatio")
            ...
            #     base, basetime = (self.analysisResults['pspSlope1'][:basepts].mean(),
            ...
            #         return
            #     postRgn = (pr1[0], pr2[-1])
            #     post, postTime = (self.analysisResults['pspSlope1'][postRgn[0]:postRgn[1]].mean(),
    ...
    def measurePSP(self, traces):
        pspRgn1 = self.pspRgn.getRegion()
        pspRgn2 = self.pspRgn2.getRegion()
        timestep = traces[0].axisValues('Time')[1] - traces[0].axisValues('Time')[0]
        #ptsToAvg = self.ctrl.measureAvgSpin.value()

        for j, r in enumerate([pspRgn1, pspRgn2]):
            for i, trace in enumerate(traces):
                data = trace['Time':r[0]:r[1]]
                ...
                self.analysisResults[i]['pspSlope%i' %(j+1)] = slope
                self.analysisResults[i]['slopeFitOffset%i'%(j+1)] = offset
                self.analysisResults[i]['highSlopeLocation%i'%(j+1)] = hightime
                ...
                baseline = data['primary'][:5].mean()
                peakPosition = np.argwhere(data['primary'] == data['primary'].max())[0]
                try:
                    avgPeak = data['primary'][peakPosition-2:peakPosition+2].mean() - baseline
                except IndexError: ## means that the peak position is at the end of the selected region and probably not the actual peak
                    avgPeak = np.nan

                self.analysisResults[i]['pspAmplitude%i'%(j+1)] = avgPeak
    ...
    def storeTrialsToDB(self):
        ...
        trialFields = OrderedDict([
            ...
            ('bridgeBalance', 'real'),
            ('pspSlope1', 'real'),
            ('normalizedPspSlope1', 'real'),
            ('slopeFitOffset1', 'real'),
            ('highSlopeLocation1', 'real'),
            ('pspAmplitude1', 'real'),
            ('pspRgnStart1', 'real'),
            ('pspRgnEnd1', 'real'),
            ('pspSlope2', 'real'),
            ('normalizedPspSlope2', 'real'),
            ('slopeFitOffset2', 'real'),
            ('highSlopeLocation2', 'real'),
            ('pspAmplitude2', 'real'),
            ('pspRgnStart2', 'real'),
            ('pspRgnEnd2', 'real'),
            ('pairedPulseRatio_slope', 'real'),
            ('pairedPulseRatio_amp', 'real'),
        ...
        data = np.zeros(len(self.analysisResults), dtype=[
                                                            ...
                                                            ('pspRgnEnd1', float),
                                                            ('pspSlope2', float),
                                                            ('normalizedPspSlope2', float),
                                                            ('slopeFitOffset2', float),
                                                            ('highSlopeLocation2', float),
                                                            ('pspAmplitude2', float),
                                                            ('pspRgnStart2', float),
                                                            ('pspRgnEnd2', float),
                                                            ('pairedPulseRatio_slope', float),
                                                            ('pairedPulseRatio_amp', float),
        ...
        data['pspRgnStart1'] = self.pspRgn.getRegion()[0]
        data['pspRgnEnd1'] = self.pspRgn.getRegion()[1]
        data['pspRgnStart2'] = self.pspRgn2.getRegion()[0]
        data['pspRgnEnd2'] = self.pspRgn2.getRegion()[1]
        data['analysisCtrlState'] = str(self.analysisCtrl.state())
        data['averageCtrlState'] = str(self.averageCtrl.state())

        baselineSlope1 = self.analysisResults[self.analysisResults['time'] < self.expStart+300]['pspSlope1'].mean()
        baselineSlope2 = self.analysisResults[self.analysisResults['time'] < self.expStart+300]['pspSlope2'].mean()
        for i in range(len(self.analysisResults)):
            ...
            for j, bs in enumerate([baselineSlope1, baselineSlope2]):
                data[i]['pspSlope%i'%(j+1)] = self.analysisResults[i]['pspSlope%i'%(j+1)]
                data[i]['normalizedPspSlope%i'%(j+1)] = self.analysisResults[i]['pspSlope%i'%(j+1)]/bs
                data[i]['slopeFitOffset%i'%(j+1)] = self.analysisResults[i]['slopeFitOffset%i'%(j+1)]
                data[i]['highSlopeLocation%i'%(j+1)] = self.analysisResults[i]['highSlopeLocation%i'%(j+1)]
                data[i]['pspAmplitude%i'%(j+1)] = self.analysisResults[i]['pspAmplitude%i'%(j+1)]
